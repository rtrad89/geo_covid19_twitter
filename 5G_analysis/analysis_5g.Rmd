...
preprocess_texts = function(texts, marker)
{
  ...
  print(paste("URLs, smilies, emojies and mentions had been removed from dataset:", marker))
  # Some cleaning:
  ...
  # Remove stand-alone numbers to get rind of 19
...
wordcloud(words = d$word, freq = d$freq, min.freq = 3, random.order=FALSE,
          max.words=150, rot.per=0.35, colors=brewer.pal(8, "Dark2"))
...
{
  # print("Sufficing with terms whose sparsity doesn't exceed .999% in all")
  # tdm.all = removeSparseTerms(x = tdm.all, sparse = .999)
  ...
  # Which words to pick? the most relevant...
  ...
  fiveG.counts = length(tdm.5g$dimnames$Docs)
  tweets.counts = length(tdm.all$dimnames$Docs)
...
for (val in unique(five_g_tweets$src))
{
  ...
  # Tweets counts
  vDoc = sort(rowSums(m),decreasing=TRUE)
  ...
  barplot(sort(salience[, "Pr.boost"], decreasing = T)[0:30],
          las = 2, cex.names = 0.7,
          main = "Top 30 terms whose conditional probability to occur in a tweet given '5g' exceed their marginal probability")
  ...
  if(!stringi::stri_startswith(str = val, fixed = "Feb"))
    break
}
