...
if(RESAVE_TDMs)
{
  ...
  saveRDS(object = dtm_all_tweets, file = file.path(DIR_TDMs_PATH,
                                                paste0(val,"ngram",NGRAMS,  "_pure_",MAX_SPARSITY,".rds")))
...
print("Salience and conditional probabilities after sufficing with 99.8% sparsity:")
dtm_all_tweets = removeSparseTerms(dtm_all_tweets, sparse = 0.998)
```
...
for (val in unique(tweets$src))
{
  if(RESAVE_INDIVIDUAL_TDMs)
  {
    ...
    dtm_tweets = TermDocumentMatrix(texts,
                                    control = list(wordLengths=c(2, Inf), tokenize = NgramTokenizer))
    saveRDS(object = dtm_tweets, file = file.path(DIR_TDMs_PATH,
                                                  paste0(val,"ngram",NGRAMS, "_pure_5G", MAX_SPARSITY,".rds")))
    ...
    dtm_all_tweets = TermDocumentMatrix(texts,
                                        control = list(wordLengths=c(2, Inf), tokenize = NgramTokenizer))
    saveRDS(object = dtm_all_tweets, file = file.path(DIR_TDMs_PATH,
                                                      paste0("ngram",NGRAMS, "_pure_", val, MAX_SPARSITY,".rds")))
  } else {
