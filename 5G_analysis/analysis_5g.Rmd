...
plot_tweets = 
  ...
  ggtitle("Number of original Eng tweets hydrated (in Thousands)",
          subtitle = paste("Re-tweets were excluded; tagged portions are black; N = ",
                           format(nrow(tweets),big.mark = ","))) +
  scale_fill_manual(values = c("FALSE"="#595959", "TRUE"="black")) +
  theme(text = element_text(size = 16), legend.position = "none") +
  geom_label(aes(src, total, fill=NULL, label=format(round(total/1000, 1), big.mark = ",")), data=tots, vjust=-0.2, size=5)
...
tweets %>%
  ...
  geom_bar(stat = "identity") +
  geom_label(aes(label=format(n_users, big.mark = ",")), size = 3) +
...
if(RESAVE_TDMs)
{
  ...
  saveRDS(object = dtm_all_tweets, file = file.path(DIR_TDMs_PATH,
                                                paste0(val, MAX_SPARSITY,".rds")))
  
} else {
  
  # Load the TDM
  dtm_tweets = readRDS(file = file.path(DIR_TDMs_PATH,
                                        paste0(val,"_5G", MAX_SPARSITY,".rds")))
...
d = d[!d$word %in% c("coronavirus", "5g"), ]
...
for (val in unique(tweets$src))
{
  ...
  print(paste0("Sustaining less than ", 100*MAX_SPARSITY, "% sprsity"))
  dtm_all_tweets = removeSparseTerms(dtm_all_tweets, sparse = MAX_SPARSITY)
...
saveRDS(object = freq.terms, file = file.path(DIR_FREQ_PATH, "freq_terms_all.rds"))
