...
RESAVE_INDIVIDUAL_TDMs = TRUE # For individual datasets
...
for (val in unique(tweets$src))
{
  if(RESAVE_INDIVIDUAL_TDMs)
  {
    ...
    dtm_all_tweets = TermDocumentMatrix(texts,
                                        control = list(wordLengths=c(2, Inf)))
    saveRDS(object = dtm_all_tweets, file = file.path(DIR_TDMs_PATH, "_pure_",
                                                      paste0(val, MAX_SPARSITY,".rds")))
  } else {
    # Loading the term-doc matrices
    dtm_tweets = readRDS(file = file.path(DIR_TDMs_PATH,
                                                  paste0(val,"_pure_5G", MAX_SPARSITY,".rds")))
    dtm_all_tweets = readRDS(file = file.path(DIR_TDMs_PATH, "_pure_",
                                                      paste0(val, MAX_SPARSITY,".rds")))
  }
...
saveRDS(object = freq.terms, file = file.path(DIR_FREQ_PATH, "_pure_freq_terms_all.rds"))
