...
PURGE_TDMs = FALSE # A dynamic flag to know if we want to purge the RDS cache repository or use what's available
...
ggsave(filename = paste0("wc_analysis_all_tweets.pdf"),
       path = "./figures/")
print(plot_tweets)
  ...
  ggtitle("Number of users of original Eng tweets",
          subtitle = paste("Re-tweets were excluded; N = ",
                           format(n_distinct(tweets$user_screen_name),
                                  big.mark = ",")))
ggsave(filename = paste0("wc_analysis_all_tweetusers.pdf"),
         path = "./figures/")
...
for (val in unique(usn$src))
{
  ...
  ggsave(filename = paste0("wc_analysistop_users_",val,".pdf"),
         path = "./figures/")
...
ggsave(filename = paste0("wc_analysis_hydrations.pdf"),
         path = "./figures/")
...
if(PURGE_TDMs)
{
  ...
  saveRDS(object = dtm_tweets, file = file.path(DIR_TDMs_PATH,
                                                paste0(val,"_5G",
                                                       MAX_SPARSITY, ".rds")))
  print("TDM of all tweets created.")  
  # MEMORY INSUFFICIENT!
  ...
  saveRDS(object = dtm_all_tweets, file = file.path(DIR_TDMs_PATH,
                                                    paste0(val,
                                                           MAX_SPARSITY, ".rds")))
  print("TDM of 5G tweets created.")
  ...
  dtm_tweets = readRDS(file = file.path(DIR_TDMs_PATH,
                                        paste0(val,"_5G", 
                                               MAX_SPARSITY, ".rds")))
  print("TDM of all tweets loaded.")
  dtm_all_tweets = readRDS(file = file.path(DIR_TDMs_PATH,
                           paste0(val,
                                  MAX_SPARSITY, ".rds")))
  print("TDM of 5G tweets loaded.")
}

# Treat n5G individually because it is idiosyncratic to this case only
n5g_fp = file.path(DIR_TDMs_PATH, paste0(val,"_n5G", MAX_SPARSITY, ".rds"))
...
print("Combined TDMS are created")
```
...
```{r Make the wordclouds of all the data, 5G and n5G}
val = "all"
print(paste("Due to technical problems, we are removing terms whose sparsity exceed", MAX_SPARSITY ,"but in 5G datasets"))
dtm_all_tweets = removeSparseTerms(x = dtm_all_tweets, sparse = MAX_SPARSITY)
...
ggtitle(paste("Top 50 Frequent Words Corpus Occurrences in Tweets (Excluding Hashtags)")) +
coord_flip()
print(p)
...
ylab("tweets") +
#easy_rotate_x_labels(angle = 90) +
ggtitle(paste("Top 50 Frequent Words Tweet Occurrences in Tweets (Excluding Hashtags)")) +
coord_flip()
...
dDoc = dDoc[!dDoc$word %in% c("5g", "coronavirus"), ]
wordcloud(words = dDoc$word, freq = dDoc$freq,
          min.freq = 3, random.order=FALSE,
          max.words=200, rot.per=0.35, colors=brewer.pal(8, "Dark2"))
...
Word clouds for 5G, n5G, and all tweets are created successfully.
