...
compute_salience = function(tdm.all, tdm.5g, sparsity=0.998, verbose = F)
{
  if(verbose)
  {
    print(tdm.all)
  ...
  print(paste("Sufficing with terms whose sparsity doesn't exceed",MAX_SPARSITY,
              "in all"))
  tdm.all = removeSparseTerms(x = tdm.all, sparse = sparsity)
...
for(val in unique(tweets.n5g$src))
{
  ...
  termfreq.n5g = vsm_modeller(data = subset(tweets.n5g, subset = src==val),
                              ...
                              sparsity_threshold = MAX_SPARSITY)[3]$termfreqs
  print("5G:")
