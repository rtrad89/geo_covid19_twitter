                        ...
                        corrterm = "coronavirus")
{
  ...
  if(!is.na(sparsity_threshold))
  {
    ...
    print(paste("NOTE: sparsity pruned, less than",
...
compute_salience = function(tdm.all, tdm.5g, verbose = F)
{
  ...
  print("Sufficing with terms whose sparsity doesn't exceed 99.8% in all")
  tdm.all = removeSparseTerms(x = tdm.all, sparse = mysparsity_threshold)
...
dtm_htgs = vsm_modeller(data = tweets.n5g, RESAVE_TDMs = RESAVE_TDMs,
                        val = "ALL",
                        sparsity_threshold = mysparsity_threshold, is5G = FALSE)
```

As we see, the current term-document matrix holds `r dtm_htgs$sparse$nrow` terms in `r dtm_htgs$sparse$ncol` tweets.

`r ifelse(is.na(mysparsity_threshold), print("") , print(paste0("Due to efficiency reason we have to suffice with the least sparse terms because we have a lot of sparsity in the TDM. That should not remove significant relations or findings we are after. Terms that are ", 100*mysparsity_threshold ,"% sparse or more will be pruned away. Removing extremely sparse terms leaves us with ", dtm_htgs$pruned$nrow , " terms.")))`
...
for(val in unique(tweets.n5g$src))
{
...
topN.n5g = visualise_evolution(terms_frequencies = freq.n5g, freq_threshold = 15,
                               label = "Non-5G")
topN.5g = visualise_evolution(terms_frequencies = freq.5g, freq_threshold = 15,
                              label = "5G")
